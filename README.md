...
Ever opened a new codebase and felt completely lost? **VisualFlow** turns any GitHub repository or local codebase into an interactive architecture map in seconds.
...
*"If I change this file, what breaks?"* — VisualFlow answers this instantly. Select any file and see exactly how many files would be affected by changes.
...
Point VisualFlow at an Obsidian vault or any markdown directory to see notes as a connected graph. Both `[[wiki-links]]` and `[text](./relative.md)` links become edges; each note is a `note`-layer node (distinct color) with a `dependencies[]` array in the JSON export.
...
<p align="center">
  <img src="./card/examples/compact.svg" alt="VisualFlow card — compact style" width="100%" />
</p>
...
**Your code stays on your machine.** VisualFlow:
...
Just visit [VisualFlow](https://VisualFlow-five.vercel.app/) and paste any GitHub URL.

### Option 2: Self-Host
```bash
# Clone the repo
git clone https://github.com/braedonsaunders/VisualFlow.git
...
1. Open VisualFlow in your browser
2. Click the "Open Folder" button
3. Select the folder or files you want to analyze
4. VisualFlow will process them entirely in your browser
...
VisualFlow extracts functions and analyzes dependencies for:
...
> VisualFlow runs entirely in your browser. It calls the GitHub API directly from your browser and processes everything client-side.
...
All examples below are real cards rendered by the [VisualFlow Card Action](./card/) against this very repo. Pick one and drop it on your README.
...
If you find VisualFlow useful, please star the repo.
